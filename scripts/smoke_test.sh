...
if echo "$PAGE_HTML" | grep -q "renderScrapeForm"; then
...
echo "$VAULT_JSON" | grep -q '"ish-d"' && ok "/api/vault has ish-d" || bad "/api/vault missing ish-d"
...
echo "$DEBUG_JSON" | grep -qi "ytdlp_version" && ok "/debug yt-dlp present" || bad "/debug missing yt-dlp"
...
# 6. Deployed source integrity (via LXC)
