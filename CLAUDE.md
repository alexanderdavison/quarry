...
- Every recent record has: id (assigned by the scraper since 2026-08-01; older ones got ids from webui's _migrate_ids at startup), title, channel, category, path (vault-relative), summary, timestamp, honcho_sync_status ("synced"|"pending"|"failed"), and since 2026-08-01 video_id + url (records written before then lack both).
- `/scrape` and `/api/batch-scrape` stream NDJSON (`application/x-ndjson`), one JSON object per line. `/scrape` yields `{type:"stage",stage,message}` then a terminal `{type:"done",...}`; `/api/batch-scrape` yields `{type:"item"|"progress"}` then `{type:"batch_done",summary}`. Both refuse duplicates before running the scraper via `_preflight_duplicate(url, video_id)`: video_id match first, then a legacy slug fallback (yt-dlp metadata → title slug vs. record path slug) for records written before video_id existed.
